...
class TestAccessControl:
    ...
    def test_anon_drop_overwritable_within_window(self, anon):
        """Anon drops are not locked — anyone can overwrite immediately."""
        key = unique_key('ac-anon-open')
        upload_text(HOST, anon, 'original', key=key, is_test=True)
        upload_text(HOST, anon, 'overwritten', key=key, is_test=True)
        _, content = get_clipboard(HOST, anon, key)
...
class TestRenameDelete:
    def test_rename_clipboard_drop(self, free_user):
        ...
        # old key gone, new key has content
        kind, _ = get_clipboard(HOST, free_user.session, key)
        assert kind is None
        kind, content = get_clipboard(HOST, free_user.session, new_key)
        assert kind == 'text' and content == 'rename me'
    ...
    def test_non_owner_cannot_delete_locked_drop(self, starter_user, free_user):
        key = unique_key('del-locked')
